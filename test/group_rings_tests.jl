@testset "cyclic_group" begin 
 ...
 @testset "group_ring" begin 
    ...
    @test RF.object == F
    @test Set(collect(b_RF)) == Set([one(F), x, x^(-1), x*y*z, (x*y*z)^(-1),
                                         x^2, x^(-2), (x*y*z)^2, (x*y*z)^(-2),
                                         x^2*y*z, x*(x*y*z)^(-1), y*z, x^(-1)*(x*y*z)^(-1),
                                         x*y*z*x, x*y*z*x^(-1), (x*y*z)^(-1)*x, (x*y*z)^(-1)*x^(-1)])                                    
    @test mstr_RF[getindex(b_RF, x),getindex(b_RF, x*y*z)] == getindex(b_RF, x^2*y*z)
    @test mstr_RF[getindex(b_RF, x*y*z),getindex(b_RF, x*y*z)] == getindex(b_RF, (x*y*z)^2)
    @test mstr_RF[getindex(b_RF, (x*y*z)^(-1)),getindex(b_RF, x^(-1))] == getindex(b_RF, (x*y*z)^(-1)*x^(-1))
    ...
    @test mstr_RF_star[getindex(b_RF_star, x),getindex(b_RF_star, x*y*z)] == getindex(b_RF_star, y*z)
    @test mstr_RF_star[getindex(b_RF_star, x*y*z),getindex(b_RF_star, x*y*z)] == getindex(b_RF_star, one(F))
    @test mstr_RF_star[getindex(b_RF_star, (x*y*z)^(-1)),getindex(b_RF_star, x^(-1))] == getindex(b_RF_star, x*y*z*x^(-1))
 end
